clear all;
%读调整参数
parameter=readtable("C:\TIMES\MCA\MCA-parameter\PEAK20");
parameter=parameter(:,2:(size(parameter,2)));
parameter=table2array(parameter);


%读原始数据
oldparameter=readtable("C:\TIMES\MCA\data\mca-peak20.xlsx");
...
for i=1:EPOCH
    ...
    %修改GEN文件
    gpathr = 'MCA-PEAK20.GEN';
    gpathw = [num2str(i),'.GEN'];
    gidr  =fopen(gpathr,'rt');
    gidw = fopen(gpathw,'wt');
    
    a0="$INCLUDE MCA-PEAK20+CHINA.DDS";
    b0="$SET RUN_NAME  'MCA-PEAK20'";
    a1=['$INCLUDE ' fpathw];
    ...
    while ~feof(ridr)
        s3= fgetl(ridr);
        s3 = strrep(s3,"MCA-PEAK20",num2str(i));
        s3 = strrep(s3,"C:\AnswerTIMESv6\Gams_WrkTI\Gamssave c:\answertimesv6\gams_wrkti\gamssave\mca-peak20",num2str(i));
        fprintf(ridw,'%s\n',s3);
    end
    fclose(ridr);
    fclose(ridw);
